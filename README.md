...
- **Everything new in v0.11.0**: the dynamic event, the seat and held-title probes the ahistorical
  holding detector reads, and the watchlist probe. All of it passes the checks and an end-to-end run
  against the simulated game, and none of it has been watched in CK3
- One question v0.11.0 asks the game rather than guessing at: whether a scope saved in the run file
  survives into an event fired from that same batch. `hd_dynamic.0001` logs the answer the first time
  it fires, and nothing depends on which way it goes
...
| **Ahistorical holdings** | The sweep now reports where each realm is *ruled from* and which duchies and kingdoms it holds where, so the Director can tell a realm holding ground on your soil from a distant court from one that merely borders you. Frontier realms are named and set aside; the rest go to the model to judge. There is no revocation and will not be — the remedy is a claim to whoever the record says should hold it, with a dynamic event to make the argument |
| **Dual triggers** | Audits no longer run only on a clock. Each one nominates three to five rulers worth watching, and a cheap yearly probe — five log lines, no completion — asks whether any of them has died, lost their primary title, or changed faith or culture. One that has interrupts the cadence and opens the next audit with what happened |
| **Window retrieval** | Alongside the general lookup, each audit asks what the record has happening in the ten years around the current date in the player's own regions, and keeps only documents that can place themselves inside that window. The queries are printed into the prompt, so "we asked and found nothing" reads differently from "nobody asked" |
...
the running game reports companion mod v0.11.0
