...
export class Director {
  ...
  async propose(snapshot, sphere, evidence) {
    const system = [
      ...
      'adjust_title_tier and grant_claim reshape rank and borders. They are the right tools when rank or borders are what diverged, and the wrong ones otherwise.',
      'A NAMED INDIVIDUAL IS ONLY WORTH SPAWNING IF THEY HAVE SOMEWHERE TO GO.',
      'A courtier with no house and no claim changes nothing: the game gains a person and the map stays exactly as it was. So do not propose a bare spawn_character to represent a dynasty that ought to hold something - give the figure the house parameter so they are genuinely of that line, and the claim parameter so their host has a claimant to press. If the record does not support a claim on any title in the snapshot, the figure is not the correction and you should propose something else or nothing at all.',
      '',
